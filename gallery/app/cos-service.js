...
class CosService {
  ...
  createObject(bucket, id, dataToUpload, mimeType, contentLength) {
    ...
    console.debug(
      `${fn}> id: '${id}', mimeType: '${mimeType}', contentLength: '${contentLength}'`
    );
  ...
  streamObject(bucket, id, res) {
    const fn = "streamObject ";
    console.debug(`${fn}> id: '${id}'`);

    this.cos
      .getObject({ Bucket: bucket, Key: id })
      .createReadStream()
      .pipe(res);
  }

  /**
   * https://ibm.github.io/ibm-cos-sdk-js/AWS/S3.html#deleteObjects-property
   * @param {*} bucket
   * @param {*} contents
   * @returns
   */
  deleteBucketObjects(bucketName, objects) {
    const fn = "deleteBucketObjects ";
    console.debug(
      `${fn}> bucket: '${bucketName}', '${objects.length}' objects to delete`
    );
    return this.cos
      .deleteObjects({
        Bucket: bucketName,
