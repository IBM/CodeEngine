...
For this sample, you can use an Cloud Object Storage bucket that you already
have. If you never used COS, then here is a one-sentence introduction: Cloud
Object Storage is a managed data service where you can store data in files.
With the following commands, you will setup your first COS instance and a
bucket. The COS bucket uses a random suffix as bucket names must be unique
across all customers. Make sure you use your own random characters.
...
sources in this repository. Feel free to use `go` or `java` or `node` or
...
ibmcloud ce job create --name ${JOB_NAME} \
  ...
  --image ghcr.io/src2img/ce-trusted-profiles-usage/go \
...
Code Engine will setup the Job and as part of that run a build of the chosen
...
token automatically and transparently.

The sample code uses that to authenticate a request against the Cloud Object
Storage API to list the items in a bucket. The response is parsed and printed.
