...
async function checkAuthn(req, res, next) {
  console.log(`performing authentication check for '${req.url}'`);
  ...
  if (!sessionToken) {
    ...
    res.status(401);
    return res.render("authfailed", viewParams);
  }
  ...
  const userDataEndpoint = process.env.OIDC_PROVIDER_USERINFO_ENDPOINT;
  console.log(`Fetching user data from '${userDataEndpoint}' ...`);
  // exchange authorization code for access token & id_token
  const response = await fetch(userDataEndpoint, opts);
...
router.get("/auth/callback", async (req, res) => {
  ...
  console.log(`Setting session cookie on domain '${process.env.COOKIE_DOMAIN}'.`);
...
// get on auth route
router.get("/auth", checkAuthn, (req, res) => {
  console.log(`handling /auth for '${req.url}'`);
  res.render("index", {
    ...viewParams,
    user: { token: req.cookies[SESSION_COOKIE], profile: JSON.stringify(req.user) },
  });
