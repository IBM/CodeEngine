...
// ws.Conn already serialises writes internally; this wrapper exists only to
// keep the WriteJSON helper consistent with the rest of the code.
...
type controlLoop struct {
	...
	// dialOpts holds common dial options (timeout is enforced via ctx).
...
func (cl *controlLoop) controlOnce(ctx context.Context) error {
	...
	for {
		...
		if err != nil {
			...
			if isCloseCode(conn.conn, err, closeCodeAgentTerminated) {
...
// isCloseCode reports whether err is io.EOF from a WebSocket close frame with
// the given code.
func isCloseCode(conn *ws.Conn, err error, code int) bool {
	return errors.Is(err, io.EOF) && ws.IsCloseError(conn, err, code)
}
...
func (cl *controlLoop) dialControl(ctx context.Context) (*controlConn, error) {
	rawURL := controlURL(cl.cfg.GatewayWSS, cl.cfg.AgentID)
	opts := &ws.DialOptions{
		ExtraHeaders: map[string]string{
			"Authorization": "Bearer " + cl.cfg.RunToken,
		},
	}
	conn, resp, err := ws.DialContext(ctx, rawURL, opts)
	if err != nil {
		if resp != nil {
...
func (cl *controlLoop) handleRelayOpen(ctx context.Context, control *controlConn, open relayOpenMessage) {
	...
	relayConn, _, err := ws.DialContext(ctx, relayURL(cl.cfg.GatewayWSS, open.RelayToken), nil)
	if err != nil {
		_ = upstream.Close()
...
// pipeFrames copies frames from src to dst opaquely, preserving opcode and
// payload bytes. It reports the first error on errCh.
func pipeFrames(src, dst *ws.Conn, errCh chan<- error) {
	for {
